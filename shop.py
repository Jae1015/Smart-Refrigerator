...
def image_recognition(image_file):
	...
	stoplist = ['Bell Pepper','Bowl','Persimmon','Fruit', 'Produce', 'Plant', 'Vegetable', 'Food','Flora','Nut','Leaf','Citrus Fruit','Grapefruit','Chair','Table','Furniture','Photo Booth','Toy','Pepper']
...
x=raw_input("Enter items you wish to check: ")
y=x.capitalize()
check.append(str(y))

while True:
	y=raw_input("Do you wish to enter another item?,If yes,press 1.")
	if (y=='1'):
		x=raw_input("Enter next item: ")
		y=x.capitalize()
		check.append(str(y))
	else:
		break
